...
function coulomb(
        ...
        cexpn::Float64, ccenter::Tuple3F64, cpower::Tuple3I64,
        dexpn::Float64, dcenter::Tuple3F64, dpower::Tuple3I64
    )
    ...
    dx, dy, dz = dcenter[1], dcenter[2], dcenter[3]              

    aI, aJ, aK = apower[1], apower[2], apower[3]
    bI, bJ, bK = bpower[1], bpower[2], bpower[3]
    cI, cJ, cK = cpower[1], cpower[2], cpower[3]
    dI, dJ, dK = dpower[1], dpower[2], dpower[3]
...
function coulomb(a::PGBF,b::PGBF,c::PGBF,d::PGBF)
    return a.NORM*b.NORM*c.NORM*d.NORM*coulomb( 
        a.expn, a.center, a.power,
        b.expn, b.center, b.power,
        c.expn, c.center, c.power,
        d.expn, d.center, d.power
    )
end

fB( i::Int64, l1::Int64, l2::Int64, p::Float64,
    a::Float64,b::Float64,r::Int64,g::Float64
  ) = binomial_prefactor(i,l1,l2,p-a,p-b)*B0(i,r,g)

B0(i::Int64,r::Int64,g::Float64) = fact_ratio2(i,r)*(4g)^(r-i)

fact_ratio2(a::Int64,b::Int64) = factorial(a)/factorial(a-2b)/factorial(b)

# using factorial(n,k) = factorial(n)/factorial(k)
