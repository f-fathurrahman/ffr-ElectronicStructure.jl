...
function c_G_to_R( Ns::Array{Int,1}, fG::Array{ComplexF64,1} )
    out = zeros(ComplexF64,Ns[1]*Ns[2]*Ns[3])
    ccall( (:fftw_inv_fft3d, "../libs/fft3d.so"), Nothing,
...
Multicolumn version
"""
function c_G_to_R( Ns::Array{Int,1}, fG::Array{ComplexF64,2} )
    Npoints = size(fG)[1]
    Ncol = size(fG)[2]
...
# --- Call FFTW3 directly
function c_R_to_G( Ns::Array{Int,1}, fR::Array{ComplexF64,1} )
    out = zeros(ComplexF64,Ns[1]*Ns[2]*Ns[3])
    ccall( (:fftw_fw_fft3d, "../libs/fft3d.so"), Nothing,
           (Ptr{ComplexF64}, Ptr{ComplexF64}, Int64, Int64, Int64),
...
Multicolumn version
"""
function c_R_to_G( Ns::Array{Int,1}, fR::Array{ComplexF64,2} )
    Npoints = size(fR)[1]
    Ncol = size(fR)[2]
    out = zeros(ComplexF64,Npoints,Ncol)
    for ic = 1:Ncol
...
function G_to_R( Ns::Array{Int,1}, fG::Array{ComplexF64,2} )
    Npoints = prod(Ns)
...
function R_to_G( Ns::Array{Int,1}, fR::Array{ComplexF64,1} )
...
function R_to_G( Ns::Array{Int,1}, fR::Array{ComplexF64,2} )
    Npoints = prod(Ns)
    Ncol = size(fR,2)
