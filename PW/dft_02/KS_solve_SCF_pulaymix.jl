function KS_solve_SCF_pulaymix( pw::PWGrid,
    ...
    for iter = 1:NiterMax
        ...
        #
        rho = pulaymix!( rho, rho_new, β, df, dv, iter, MIXDIM )
        ...
        @printf("SCF_pulaymix: %8d %18.10f %18.10e %18.10e\n", iter, Etot, diffE, diffRho )        

        if diffE < 1e-7
            @printf("SCF_pulaymix is is converged: iter: %d , diffE = %10.7e\n", iter, diffE)
            break
        end
        #
        Etot_old = Etot
