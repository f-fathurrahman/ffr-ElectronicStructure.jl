...
function test_main( ; pot_type = "harmonic", method = "Emin_cg" )
    # LF parameters
    NN = [25, 25, 25]
    AA = [0.0, 0.0, 0.0]
    BB = [6.0, 6.0, 6.0]
    ...
    if method == "Emin_cg_sparse"
        ...
        Energies, evecs, Potentials = KS_solve_Emin_pcg( LF, Gv, ∇2, precH,
                                        V_ionic, Focc, Nstates, verbose=true )
    ...
    elseif method == "SCF"
        ...
        precH = prec_mkl_ilu0( -0.5*∇2 + spdiagm(V_ionic) )
        #precH = speye(Npoints)
        Energies, evals, evecs, Potentials = KS_solve_scf( LF, Gv, ∇2, precH, 
                                               V_ionic, Focc, Nstates, verbose=true )
        #
    else
    #
        Energies, evecs, Potentials = KS_solve_Emin_cg( LF, Gv, V_ionic, Focc, Nstates, verbose=true )
